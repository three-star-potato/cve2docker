...
def parse_dependency_paths(lines):
    ...
    for line in lines:
        ...
        product_dependence = []
        try:
            items = eval(line.split(':')[1].strip())
        except:
            continue

        for item in items:
            if isinstance(item, list):
                product_dependence.append(item[0])
            else:
                product_dependence.append(item)
        print(len(product_dependence))
        dependencies[product_name] = product_dependence
    
    return dependencies

graph_size_require = read_products('node_size_require.json')
depend_path='node_dependency_paths.txt'
dependencies = {} 
...
print(f"Size difference after merging for Beam_Search_1: {merged_size_1 - size_1}")
print(f"Size difference after merging for Beam_Search_2: {merged_size_2 - size_2}")
print(f"Size difference after merging for Beam_Search_3: {merged_size_3 - size_3}")
